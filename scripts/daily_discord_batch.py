...
import os, sys, io, re, time, json, base64, random, hashlib, warnings, traceback
import requests
from urllib.parse import urlencode
from PIL import Image, ImageOps, ImageFilter

# Silence the noisy torch/easyocr warnings on runners
warnings.filterwarnings("ignore", message=r".*pin_memory.*", module="torch.*")
warnings.filterwarnings("ignore", category=UserWarning)

# Optional OCR backends (free)
try:
    import pytesseract
except Exception:
    pytesseract = None
...
REDEEM_MAX_ATTEMPTS  = int(os.environ.get("REDEEM_MAX_ATTEMPTS", "3"))
CAPTCHA_MAX_REFRESH  = int(os.environ.get("CAPTCHA_MAX_REFRESH", "4"))
MAX_FIDS             = int(os.environ.get("MAX_FIDS", "0"))   # 0 = no cap
MAX_CODES            = int(os.environ.get("MAX_CODES", "0"))
DEBUG                = os.environ.get("DEBUG", "0") == "1"
SAVE_CAPTCHAS        = os.environ.get("SAVE_CAPTCHAS", "0") == "1"
...
if not all([BOT_TOKEN, CODES_CH, IDS_CH, STATE_CH, WOS_SECRET]):
    ...
    sys.exit(0)  # keep workflow green while iterating

# ========== DISCORD REST ==========
D_API = "https://discord.com/api/v10"
D_HDR = {"Authorization": f"Bot {BOT_TOKEN}"}

def post_message(channel_id, content) -> int:
    r = requests.post(
...
def get_messages_after(channel_id, after_id=None, limit=100):
    ...
    if not r.ok:
        print(f"[ERR] Discord GET ch={channel_id} status={r.status_code} body={r.text[:200]}")
        return []
    try:
        return r.json()
    except Exception:
...
def parse_codes(text):
    ...
    for line in text.splitlines():
        ...
        m = BULLET.match(line)
        if m and cur == "codes":
            out.add(m.group(1).strip().upper()); continue
...
def parse_fids(text):
    ...
    for line in text.splitlines():
        ...
        m = BULLET.match(line)
        if m and cur == "fids":
            out.add(m.group(1).strip()); continue
...
def sign_with_captcha(fid: str, cdk: str, captcha: str, ts_ms: str) -> str:
    # Matches observed frontend: exact order with captcha included
...
class CaptchaSession:
    ...
    def __enter__(self):
        ...
        for sel in ("img.verify_pic", "div.verify_pic_con img", "img.reload_btn", ".reload_btn"):
            try: self.page.wait_for_selector(sel, timeout=4000)
    ...
    def refresh_captcha(self):
        try:
            pic = self.page.query_selector("img.verify_pic") or self.page.query_selector("div.verify_pic_con img")
            ...
            if btn:
                ...
                if old_src:
                    self.page.wait_for_function(
                        "(oldSrc)=>{const el=document.querySelector('img.verify_pic,div.verify_pic_con img');return el && el.src && el.src!==oldSrc;}",
        ...
        try:
            ...
            self.page.wait_for_selector("img.verify_pic,div.verify_pic_con img", timeout=6000)
        except Exception:
            self.page.wait_for_timeout(800)

# -------- OCR helpers (free) --------
def _tesseract_read(png: bytes) -> str:
...
_EASY = None
def _easyocr_read(png: bytes) -> str:
    global _EASY
    ...
    try:
        ...
        for _, text, conf in res:
            ...
            if 3 <= len(s) <= 8 and float(conf or 0) > best_conf:
                best_txt, best_conf = s, float(conf or 0)
        return best_txt
    except Exception:
        return ""

_saved_caps = 0
def maybe_post_captcha(sample_bytes: bytes, guess: str):
    global _saved_caps
    if not SAVE_CAPTCHAS or not sample_bytes or _saved_caps >= 5:
        return
    _saved_caps += 1
    fname = f"captcha_{int(time.time())}_{guess or 'unknown'}.png"
    post_message_with_file(STATE_CH, f"Captcha sample (OCR guess: `{guess}`)", fname, sample_bytes)

def solve_captcha_ensemble(png: bytes) -> str:
    if not png: return ""
    t = _tesseract_read(png)
    e = _easyocr_read(png)
    if DEBUG:
        print(f"[DBG] OCR tesseract={t!r} easyocr={e!r}")
    if t and e and len(t) == len(e) and sum(1 for a, b in zip(t, e) if a == b) >= max(3, len(t) - 1):
        guess = t
    else:
        guess = (max([t, e], key=lambda s: (len(s or ""), s is not None)) or "")[:6]
    maybe_post_captcha(png, guess)
    return guess

def get_working_captcha(sess: CaptchaSession, max_refresh: int | None = None) -> str:
    if max_refresh is None:
        max_refresh = CAPTCHA_MAX_REFRESH
...
try:
    ...
    # Optional caps while testing
    sid_list = sorted(roster.keys())
    if MAX_FIDS > 0: sid_list = sid_list[:MAX_FIDS]
    code_list = sorted(codes)
    if MAX_CODES > 0: code_list = code_list[:MAX_CODES]
    ...
    with CaptchaSession() as sess:
        ...
        # ---- redeem loop (fresh captcha every attempt) ----
        ...
        for code in code_list:
            ...
            for fid in sid_list:
                ...
                for attempt in range(REDEEM_MAX_ATTEMPTS):
                    captcha_code = get_working_captcha(sess)
                    if not captcha_code:
                        status = "NO_CAPTCHA"
                        break

                    http, msg = redeem_once(fid, code, captcha_code)
                    if DEBUG and attempt == 0:
                        print(f"[DBG] {fid} {code} attempt={attempt+1} http={http} msg={msg[:80]}")
                    ...
                    elif ("CAPTCHA" in msg) or ("VERIFY" in msg) or ("VERIFICATION" in msg):
                        status = "CAPTCHA_RETRY"
                        time.sleep(0.6); continue
                    elif http == 429 or "TOO MANY" in msg:
                        status = "RATE_LIMIT"
                        time.sleep(2.5 + random.uniform(0, 0.8)); continue
                    elif ("SIGN" in msg) or ("PARAMS" in msg) or http in (400, 405, 422):
                        status = "CAPTCHA_RETRY"
                        time.sleep(0.6); continue
                    else:
...
if 'codes' in locals() and locals().get("codes"):
    parts.append("**Codes processed**\n" + ", ".join(f"`{c}`" for c in sorted(locals().get("codes"))))
...
if 'redeem_lines' in locals() and locals().get("redeem_lines"):
    parts.append("**Redeem results (first 25)**\n" + "\n".join(locals().get("redeem_lines")[:25]) + (" \n…" if len(locals().get("redeem_lines")) > 25 else ""))
