import os, sys, json, re, time, io, base64, hashlib, traceback, random, warnings
import os, sys, json, re, time, io, base64, hashlib, traceback, random
import requests
from urllib.parse import urlencode
from PIL import Image, ImageOps, ImageFilter

# Optional OCR libs (free, no external paid solver)
try:
import pytesseract
except Exception:
pytesseract = None
try:
    import easyocr  # pulls torch; we'll silence its warnings
except Exception:
    easyocr = None

# Silence the long torch dataloader "pin_memory" warnings
...
WOS_SECRET = os.environ.get("WOS_SECRET", "").strip()  # still used for player lookups (no captcha)
...
if not all([BOT_TOKEN, CODES_CH, IDS_CH, STATE_CH, WOS_SECRET]):
    ...
    sys.exit(0)  # soft exit so the workflow stays green
...
@@ -84,11 +80,11 @@ def find_latest_state_message(channel_id, bot_user_id):
try:
...
@@ -115,53 +111,68 @@ def parse_fids(text):
...
# -------- Captcha browser session (now logs in per FID) --------
...
self.browser = self.play.chromium.launch(headless=True)
        ...
        # Initial screen should have the Player ID input + Login button
        try:
            self.page.wait_for_selector("input[placeholder*='Player ID']", timeout=8000)
        except Exception:
            pass
return self

def __exit__(self, exc_type, exc, tb):
@@ -170,49 +181,17 @@ def __exit__(self, exc_type, exc, tb):
try: self.play.stop()
except: pass
    ...
    # NEW: login with a specific FID so captcha appears
    def login_fid(self, fid: str) -> bool:
        try:
            # Ensure we are on the home screen (if not, reload)
            if not self.page.locator("input[placeholder*='Player ID']").first.is_visible():
                self.page.goto(REFERER, wait_until="domcontentloaded", timeout=35000)

            # Fill FID and click Login
            self.page.fill("input[placeholder*='Player ID']", fid)
            # The "Login" control can be a button or a div/span; click by text.
            self.page.locator("text=Login").first.click()
        except Exception:
            # As a fallback, try pressing Enter in the Player ID field
            try:
                self.page.press("input[placeholder*='Player ID']", "Enter")
            except Exception:
                pass

        # Wait for the post-login layout where captcha area is present
        try:
            # The verify image or its container appears only after login
            self.page.wait_for_selector("img.verify_pic, .verify_pic_con img", timeout=8000)
            return True
        except Exception:
            return False
    ...
    def captcha_bytes(self) -> bytes | None:
        """
        Pull the captcha image after login:
        1) read base64 from <img class="verify_pic" src="data:image/...">
        2) else take an element screenshot
        """
    @staticmethod
    def _get_base64_or_screenshot(page):
        # Return bytes for the captcha image (prefer base64 data URI from <img.verify_pic>)
try:
            el = self.page.query_selector("img.verify_pic") or self.page.query_selector(".verify_pic_con img")
            el = page.query_selector("img.verify_pic") or page.query_selector("div.verify_pic_con img")
if el:
src = el.get_attribute("src") or ""
if src.startswith("data:image"):
@@ -223,147 +202,185 @@ def captcha_bytes(self) -> bytes | None:
pass
return None

    def refresh_captcha(self):
        """
        Click the reload icon or reload page section to get a new captcha.
        """
    @staticmethod
...
def easyocr_read(png: bytes) -> str:
    ...
    try:
        res = _EASY_READER.readtext(png, detail=1)
        best_txt, best_conf = "", 0.0
        for _, text, conf in res:
            s = re.sub(r'[^A-Za-z0-9]','', text or '').upper()
            if 3 <= len(s) <= 8 and float(conf) > best_conf:
                best_txt, best_conf = s, float(conf)
        return best_txt
    except Exception:
        return ""
...
# ========== MAIN ==========
post_message(STATE_CH, "🟢 WOS daily run (free OCR, with login-per-FID) starting…")
...
if not furnace_ups:
@@ -372,81 +389,68 @@ def get_working_captcha(sess: CaptchaSession, max_refresh=4) -> str:
furnace_snap.append(f"`{fid}` • L{rec['stove']} {rec.get('nickname') or ''}")
furnace_snap.sort()

    # Redeem loop: **login once per FID**, then redeem all codes with captcha refresh as needed
...
redeem_lines = []
    ...
    with CaptchaSession() as sess:
        for fid in sorted(roster.keys()):
            if not sess.login_fid(fid):
                # could not reach captcha area for this FID; skip with marker
                for code in sorted(codes):
                    safe = code[:3]+"…" if len(code)>3 else code
                    redeem_lines.append(f"❌ {fid} • {safe} • NO_LOGIN")
                    fail_redeems += 1
                continue

            cookie_hdr = sess.cookie_header()
            captcha_code = get_working_captcha(sess)
            if DEBUG:
                post_message(STATE_CH, f"[DEBUG] FID {fid}: initial captcha {'OK' if captcha_code else 'MISS'}")

            def redeem_once(code: str, captcha: str):
                ...
                form["sign"] = md5(f"fid={fid}&cdk={code}&captcha_code={captcha}&time={ts_ms}" + WOS_SECRET)
...
status = "UNKNOWN"
                ...
                # Try up to 3 new captchas if we get sign/params/captcha related errors
                for _ in range(3):
                    if not captcha_code:
                        status = "NO_CAPTCHA"; break

                    http, msg = redeem_once(code, captcha_code)
                    ...
                    if "SUCCESS" in msg:
                        ...
                        status = "RATE_LIMIT"
                        time.sleep(2.2 + random.uniform(0,0.8))
                        continue
                    elif "CAPTCHA" in msg or "SIGN" in msg or "PARAMS" in msg or http == 405:
...
@@ -461,10 +465,9 @@ def redeem_once(code: str, captcha: str):
except Exception as e:
...
@@ -481,7 +484,7 @@ def redeem_once(code: str, captcha: str):
